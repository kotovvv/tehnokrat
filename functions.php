<?php namespace BytePerfect\WordPress\Theme\Tehnokrat;
...
final class Tehnokrat {
	...
	private function __construct() {
		...
		if ( is_admin() ) {
			new Bulk_SMS_Sender(
				$this->sms_sender,
				apply_filters( 'tehnokrat_settings.components.sms.bulk_messages',
...
//		add_filter( 'sanitize_title', [ $this, 'remove_cyrillic_symbols' ], - PHP_INT_MAX );
		...
		if ( is_admin() ) {
			...
			add_filter( 'woocommerce_get_formatted_order_total', [ $this, 'get_formatted_order_total' ], 10,
				2 );
			add_action( 'admin_enqueue_scripts', [ $this, 'admin_enqueue_scripts' ], 20 );

			add_filter( 'manage_product_cat_custom_column', [ $this, 'fix_product_cat_reorder' ],
				- PHP_INT_MAX );
		} else {
			add_action( 'wp_footer', [ $this, 'load_vue_templates' ], - PHP_INT_MAX );
			remove_action( 'wp_footer',
				'woocommerce_demo_store' ); // remove default woocommerce demo store notice
...
//		add_filter( 'woocommerce_get_order_item_totals', [ $this, 'amend_order_item_totals' ] );
		add_action( 'woocommerce_after_checkout_validation', [ $this, 'validate_billing_phone' ], PHP_INT_MAX,
			2 );
		...
		add_action( 'woocommerce_order_status_pending_to_processing_notification',
			[ $this, 'disable_cod_processing_notification' ], - PHP_INT_MAX, 2 );
					...
					while ( $data = fgetcsv( $handle ) ) {
						...
						if ( $post_id ) {
							foreach (
								...
								$meta_ids = $wpdb->get_var( $wpdb->prepare( $get_meta_id_query, $meta_key,
									$post_id ) );
								if ( empty( $meta_ids ) ) {
									$wpdb->insert(
										$wpdb->postmeta,
						...
						if ( $product = wc_get_product( $product_id ) ) {
							if ( $product_attribute_slugs = array_filter( array_slice( $data, 3 ) ) ) {
								...
								foreach ( $wpdb->get_results( $query ) as $attribute ) {
									...
									$attributes[ $attribute->slug ]->set_position( array_search( $attribute->slug,
								...
								$logger->error( "Product ID <{$product_id}>: empty attributes list!",
									$context );
							}
						} else {
...
//			'product/apple-imac-21-retina-4k-z0tk000r6-mndy24-mid-2017-3-0ghz16gb1tb-hdd555$' => 'index.php?product_cat=mac/imac',
		...
		if ( $options_redirect_map = $this->get_setting( 'redirect_map', 'redirect_map' ) ) {
			...
			foreach ( $options_redirect_map as $urls ) {
				if ( empty( $redirect_map[ $urls['url_new'] ] ) ) {
					$response   = wp_remote_get( $urls['url_new'],
				...
				$url_404                           = trim( str_replace( site_url( '/' ), '',
						$urls['url_404'] ), '/' ) . '$';
	...
	public function manage_shop_order_columns( $columns ) {
		...
		$ref_pos     = 2;
		$columns     = array_slice( $columns, 0, $ref_pos + 1, true ) + $order_items + array_slice( $columns,
				$ref_pos + 1, count( $columns ) - 1, true );
	...
	public function show_order_items( $column ) {
		...
		if ( 'order_items' == $column ) {
			...
			echo '<a href="#" class="show_order_items">' . apply_filters( 'woocommerce_admin_order_item_count',
					sprintf( _n( '%d item', '%d items', $the_order->get_item_count(), 'woocommerce' ),
						$the_order->get_item_count() ), $the_order ) . '</a>';
			...
			if ( sizeof( $the_order->get_items() ) > 0 ) {
				...
				foreach ( $the_order->get_items() as $item ) {
					$product        = apply_filters( 'woocommerce_order_item_product', $item->get_product(),
						$item );
					$item_meta      = ( WC()->version < '3.1.0' ) ? new \WC_Order_Item_Meta( $item ) : new \WC_Order_Item_Product;
					$item_meta_html = ( WC()->version < '3.1.0' ) ? $item_meta->display( true,
						true ) : $item_meta->get_product();
					...
					<tr class="<?php echo apply_filters( 'woocommerce_admin_order_item_class', '', $item,
						...
						<td class="name">
							...
							<?php else : ?>
								<?php echo apply_filters( 'woocommerce_order_item_name', $item->get_name(),
									$item, false ); ?>
							<?php endif; ?>
		...
		if ( ! doing_filter( 'woocommerce_email_order_details' ) ) {
			if ( $rate = get_post_meta( $order->get_id(), '_usd_to_uah', true ) ) {
				...
				if ( 1544310000 < $order_date->getTimestamp() ) {
					...
					$formatted_total .= '</br>' . wc_price( ceil( $order->get_total() * $rate ),
