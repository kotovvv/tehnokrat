...
const Filter = memo(({ stateFilter, changeFilter, products }) => {
  ...
  useEffect(() => {
    changeFilter({
      ...stateFilter,
      setMin: inputs.input1,
      setMax: inputs.input2,
    });
  }, [inputs]);
  ...
  function setPrice(e) {
    e.preventDefault();
    const setmin = inputs.input1;
    const setmax = inputs.input2;
  ...
  function checkAttr() {
    const selectedAttr = [...document.querySelectorAll('input[name=attr]:checked')].reduce((acc, el) => {
      const atr = el.getAttribute('atr');
      const value = el.value;
      const foundAttr = acc.find((fe) => fe.id === atr);

      if (foundAttr) {
        foundAttr.values.push(value);
      } else {
        acc.push({
          id: atr,
