...
const TypeDisplay = memo(({ inDisplay, switchInDisplay, container }) => {
  ...
  return createPortal(<div className="type-display">
    <a href={new URL(window.location.origin + window.location.pathname + '?pl=bloks')} onClick={(e) => { e.preventDefault }}><input id="bloks" type="radio" name="typedisplay" value="bloks" checked={inDisplay === 'bloks'} onChange={switchInDisplay} />
    ...
    <a href={new URL(window.location.origin + window.location.pathname)} onClick={(e) => { e.preventDefault }}>
